# arreglar lambda para que se pase como una lista!
# arreglar P      para que se pase como una lista!
# si se pasa directamente el x en una base
# incluir penalizaciones en la viñeta
# incluir smooth bassis y
# incluir penalizacion en las vnf
# renombrar a fregre.lm? incorporar GLS?
fregre.lm.func=function(formula,data,basis.x=NULL,basis.b=NULL,lambda=0,P=c(1,0,0),...){
 ...
 if (any(names(data)==response)) {
    ...
    y3<-data[[response]]$data       # si es de la clase fd??
#       print(paste("Functional response:",response))
       XX[[response]]<-data[[response]][["data"]]
       tty<-data[[response]][["argvals"]]
       rtty<-data[[response]][["rangeval"]]
       namy<-data[[response]][["names"]]
       n<-nrow(y3)
...
#print("aaaa                   bbbbbbbbbbbbbbbbb")
     ...
     RR[[vfunc[i]]]<-P.penalty(l,P)# diag(lenl)
     nn<-c(nn,lenl )
        XX[[vfunc[i]]] = Z
...
#print(z)      
      ...
      if (isfdata) { z$residuals<-fdata(z$residuals,tty,rtty,namy)
...
# print(name.coef2)
#print(coeff)      
         if (isfdata) {   beta.l[[vfunc[i]]]=fd((coeff[name.coef2,]),basis.b[[vfunc[i]]])
...
if (isfdata){  sume<-colSums(z$residuals$data^2)
