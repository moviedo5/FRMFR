# sintaxis que permita np(),pc(),bsp()
# sintaxis que permita (X-h,x,x+h)
# opt 1 todos medidos en el mismo t, solo basis y no 
# opt 2 todos medidos en el mismo [a,b] pero las x's sparse
# paralelizar el for para que vaya + rapido
# rest=sum(norm.fdata(newy -func.mean(y))^2)
# 1-sum(norm.fdata(newy -pred0)^2)/rest  # puede salir negativo
# modificar ayuda
...
#' rest=sum(norm.fdata(newy -func.mean(y))^2)
# sum(norm.fdata(newy -pred0)^2)
# sum(norm.fdata(newy -pred1)^2)
# sum(norm.fdata(newy -pred2)^2);sum(norm.fdata(newy -pred3)^2)
...
# print(speci[i])      
  ...
  for (i in 1:npy){
    ...
    pf0<-pf
      ...
      if (h == 0)         { 
        for (j in 1:lenfunc){
          nam.var <- paste(vfunc[j],".",i,sep="",collpase="")
        if (specials2[j] != "0") {
        ...
        # Xmat <- cbind(Xmat,data[[vfunc[j]]]$data[,hh])
        Xmat[nam.var] <- data[[vfunc[j]]]$data[,hh[[i]]]
        }
      }
...
# xxx <- ldat$x$data[,1:4]
# colnames(xxx)<-c("x1","x2","x3","y")
# class(xxx)
# res <- gam(y~s(x1)+s(x2),data=as.data.frame(xxx))
# summary(res)
# plot(res)
# res2 <- gam(y~s(x1,x2),data=as.data.frame(xxx))
# summary(res)
# plot(res)
# ################## FIXED BASIS
# plot(res$residuals-res2$residuals)
# library(fda.usc)
...
# nbasis.x=19;nbasis.y=15
# ldat=ldata("df"=dataf,"x"=x,"x.d1"=x.d1,"x.d2"=x.d2)
# res=fregre.gsam.cl(x.d2~+s(x),ldat,h=2)
#  newy  <- ldat2$y
# y <- ldat$y
# 
# rest=sum(norm.fdata(newy -func.mean(y))^2)
# rs2p=1-sum(norm.fdata(newy -pred)^2)/rest
