...
#' @param basis.s Basis related with \code{s}(covariate) and it is used in the estimation
#' of \eqn{\beta(s,t)}.
#' @param basis.t Basis related with \code{t}(response) and it is used in the estimation
#' of \eqn{\beta(s,t)}.
...
#' \item \code{y}{ \code{fd} object representing Functional response.} 
#' \item \code{x}{ \code{fd} object representing Functional covariate.} 
...
n=ncurves = ncol(xfdobj$coef) #coefdx[2]
xcoef = xfdobj$coef
ycoef = yfdobj$coef
################################################################################
...
alphafdPar = fdPar(basis.t,Lfdobj.t, lambda.t)
...
if (is.null(weights)) W=diag(1,n) else W=diag(weights)
xmcoef<-Data2fd(argvals =x$arg, y = t(func.mean(x)$data), basisobj = basis.s,...)$coefs
x2coef=sweep(xcoef,1,xmcoef,"-")
xmd=drop(t(xmcoef)%*%basis.ss%*%Minverse(x2coef%*%W%*%t(x2coef)%*%basis.ss)%*%x2coef%*%W)
Ha=diag(1/n-xmd,n)
H=Ha+t(xcoef)%*%basis.ss%*%Minverse(xcoef%*%W%*%t(xcoef)%*%basis.ss)%*%xcoef%*%W
# No se devuelve porque da problemas al no estar los datos centrados
out = list(call = call, alpha.est = alphafd, coefficients = beta.est, 
         ...
         x = xfdobj, y = y, yfd = yfdobj, H = Hinprod, basis.s = basis.s, basis.t = basis.t, 
         argvals.y = tty, betaspenmat= R, betatpenmat= S,alphapenmat=Q)
class(out)<-"fregre.fr"
return(out)
}
