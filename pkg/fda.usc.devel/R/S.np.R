...
res <- matrix(1/ncol(tt), nrow=nrow(tt), ncol=ncol(tt))
}
return(res)} # nolint
...
S.LCR<-function(tt, h, Ker=Ker.norm, w=NULL, cv=FALSE){
  res <- S.LPR(tt=tt, h=h, p=3, Ker=Ker, w=w, cv=cv)
  return(res)
}
...
S.KNN<-function(tt,h=NULL,Ker=Ker.unif,w=NULL,cv=FALSE){
  ...
  rr=matrix(1/ncol(tt),nrow=nrow(tt),ncol=ncol(tt))	
  }
...
S.NW<-function (tt, h=NULL, Ker = Ker.norm,w=NULL,cv=FALSE) {
  ...
  S =matrix(1/ncol(tt),nrow=nrow(tt),ncol=ncol(tt))	
	}
