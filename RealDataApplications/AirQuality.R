...
# ------------------------------------------------------------
# Load Air Quality data set and basic inspection
# ------------------------------------------------------------     
data("AirQuality")
names(AirQuality)
...
al <- lapply(listfinal[-1],function(v) which(is.na(v)))
al <- unique(rapply(al,drop))

# Remove rows containing any missing curves across variables
listfinal <- subset(listfinal,-al)
dim(listfinal$df)

# ------------------------------------------------------------
# Quick exploratory plots (curves + functional mean)
# ------------------------------------------------------------
par(mfrow = c(2, 3))
...
# ------------------------------------------------------------
# Distance correlation matrix between functional variables
# ------------------------------------------------------------
...
# Example submatrix (chosen rows/cols)
irow <- c(3, 2, 4, 6, 8)
icol <- c(1, 3, 2, 4, 6)
round(mdcor[irow, icol], 3)

# ------------------------------------------------------------
# Train/predict comparison across methods
# ------------------------------------------------------------
computeFAMM <- TRUE  # this can be time consuming
trace <- FALSE # used in fregre.kam.fr
...
for (i in 1:nrep){
  ...
  # PCA bases for response and covariates
  b.y <- create.pc.basis(ltrain$C6H6)
  b.x <- list(
    CO   = create.pc.basis(ltrain$CO,   1:5),
  ...
  # Metric choices per variable for KAM (kernel additive model)
  pmetric <- list(
    C6H6 = list(metric = metric.lp, lp = 2),
    CO   = list(metric = metric.lp),
    NMHC = list(metric = metric.lp, lp = 2),
    NOx  = list(metric = metric.lp),
    NO2  = list(metric = metric.lp),
    O3   = list(metric = metric.lp),
    Temp = list(metric = metric.lp),
    RH   = list(metric = metric.lp)
  )
  # Bandwidth grid for NP inner fits (if used inside KAM)
  lprob  <- c(seq(0.005, .1, len = 51), .2, 0.3, .4, 0.5, .6, .7)
  par.np <- list(
    CO   = list(h = c(h.default(ltrain$CO,   prob = lprob), Inf)),
  ...
  # --- Fit FRM models (linear, smooth additive, kernel additive) ---
  ...
  mod.kam.fr <- fregre.kam.fr(C6H6~NMHC, data=ltrain, 
                              par.metric=pmetric, 
                              par.np=par.np, 
  ...
  # ------------------------------------------------------------
  # Competitors: refund::pffr (PC-based and nonlinear sff terms)
  # ------------------------------------------------------------
  tj         <- 0:23 + .5
  s.n.basis  <- 11
  t.n.basis  <- 11
  ...
  if (computeFAMM){
    ...
    predsff=fdata(predict(mod.pffnl,
                          list(X1=lpred$NMHC$data,
                               X2=lpred$O3$data,
                               X3=lpred$CO$data,
                               X4=lpred$NO2$data,
                               X5=lpred$NOx$data)),
  ...
  predpffr <- fdata(predict(mod.pffr,list(X1=lpred$NMHC$data,
                                       X2=lpred$O3$data,
                                       X3=lpred$CO$data,
                                       X4=lpred$NO2$data,
                                       X5=lpred$NOx$data)),
  ...
  fitt <- fdata(matrix(mod.pffr$fitted.values,ncol=length(tj),byrow=TRUE),
                argvals=tj,
                rtt)
  ...
  # ------------------------------------------------------------
  # Competitors: FRegSigCom (linear and nonlinear)
  # ------------------------------------------------------------
  XX=list(X1=X1
  #		  ,X2=X2,X3=X3
  #		  ,X4=X4,X5=X5
  ...
  mod.SCLin <- cv.sigcom(XX, YD, t.x.list, tj, 
                         s.n.basis = s.n.basis, 
                         t.n.basis = t.n.basis)
  mod.SCNL  <- cv.nonlinear(XX, YD, t.x.list, tj, 
                            s.n.basis = s.n.basis,
                            x.n.basis = x.n.basis, 
                            t.n.basis = t.n.basis)
  			
  predSCLin <- fdata(pred.sigcom(mod.SCLin,XXnew),tj,rtt)
  predSCNL <- fdata(pred.nonlinear(mod.SCNL,XXnew),tj,rtt)  
  fitt3 <- fdata(pred.sigcom(mod.SCLin,XX),tj,rtt)
  fitt4 <- fdata(pred.nonlinear(mod.SCNL,XX),tj,rtt)
