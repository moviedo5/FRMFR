...
computeFAMM <- FALSE #Time consuming
...
for (i in 1:nrep){
  ...
  predsff <- fdata(predict(mod.pffnl,list(X1=lpred$NMHC$data,X2=lpred$O3$data,
                   ...
                   argvals=tj,c(0,24))
  predpffr <- fdata(predict(mod.pffr,list(X1=lpred$NMHC$data)),argvals=tj,c(0,24))
  ...
  predpffr <- fdata(predict(mod.pffr,list(X1=lpred$NMHC$data,X2=lpred$O3$data,
                                            X3=lpred$CO$data)),argvals=tj,c(0,24))
  ...
  predpffr <- fdata(predict(mod.pffr,list(X1=lpred$NMHC$data,X2=lpred$O3$data,
                                          ...
                                          X5=lpred$NOx$data)),argvals=tj,c(0,24))
  fitt <- fdata(matrix(mod.pffr$fitted.values,ncol=length(tj),byrow=TRUE),argvals=tj,c(0,24))
  ...
  if (computeFAMM){
    ...
    fitt <- fdata(matrix(mod.pffnl$fitted.values,
                         ncol=length(tj),byrow=TRUE),argvals=tj,c(0,24))
  ...
  mtrain1[i,6] <- 1 - mean(norm.fdata(ytrain - fitt3)^2)/MSEtr
  mtrain1[i,7] <- 1 - mean(norm.fdata(ytrain - fitt4)^2)/MSEtr
  ...
  fitt3 <- fdata(pred.sigcom(mod.SCLin,XX),tj,c(0,24))
  fitt4 <- fdata(pred.nonlinear(mod.SCNL,XX),tj,c(0,24))
  mtrain2[i,6] <- 1 - mean(norm.fdata(ytrain - fitt3)^2)/MSEtr
  mtrain2[i,7] <- 1 - mean(norm.fdata(ytrain - fitt4)^2)/MSEtr
  ...
  fitt3 <- fdata(pred.sigcom(mod.SCLin,XX),tj,c(0,24))
  fitt4 <- fdata(pred.nonlinear(mod.SCNL,XX),tj,c(0,24))
  mtrain3[i,6] <- 1 - mean(norm.fdata(ytrain - fitt3)^2)/MSEtr
  mtrain3[i,7] <- 1 - mean(norm.fdata(ytrain - fitt4)^2)/MSEtr
