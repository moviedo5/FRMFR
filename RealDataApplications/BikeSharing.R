...
# -------------------------------------------------------------------
# Packages
# (old version until 2025/11/14) # library(fda.usc.devel)
...
plot(x1, col = gray(0.5))
lines(x1[lesp], lwd = 2)
plot(x2, col = gray(0.5))
lines(x2[lesp], lwd = 2)
plot(x3, col = gray(0.5))
lines(x3[lesp], lwd = 2)
plot(x4, col = gray(0.5))
lines(x4[lesp], lwd = 2)
plot(y, col = gray(0.5))
lines(y[lesp], lwd = 2)
# dev.off()
...
for (i in 1:nrep){
  ...
  # ---- fda.frm models
  b.x <- list(
    X1 = create.pc.basis(X1, 1:npc1),
    X2 = create.pc.basis(X2, 1:npc2),
    X3 = create.pc.basis(X3, 1:npc3),
    X4 = create.pc.basis(X4, 1:npc4)
  )
  b.y <- create.pc.basis(yt, 1:npcy)
  
  pmetric <- list(
    df = data.frame(idx = 1:nrow(yt)),
    yt = list(metric = metric.lp, lp = 2),
    X1 = list(metric = metric.lp, lp = 2),
    X2 = list(metric = metric.lp, lp = 2),
    X3 = list(metric = metric.lp, lp = 2),
    X4 = list(metric = metric.lp, lp = 2)
  )
  p.np <- list(
    X1 = list(Ker = AKer.norm),
    X2 = list(Ker = AKer.norm),
    X3 = list(Ker = AKer.norm),
    X4 = list(Ker = AKer.norm)
  )
  
  
  # Examples with subsets of covariates 
  ...
  if (computeFAMM){
    respff1=pffr(Y1D~sff(X4D, xind = tj, 
                         splinepars = list(bs ="ps", m = c(2, 2, 2), k = 7))
    ...
    respff1.fit <- fdata(matrix(respff1$fitted.values, ncol = length(tj), 
                                byrow = TRUE), argvals = tj)
  ...
  respff1pc <- pffr(Y1D ~ ffpc(X4D, xind = tj, 
                               npc.max = npc4, 
                               splinepars = list(k = 11))
                 #                  +ffpc(X2D,xind=tj,npc.max=npc2,splinepars=list(k=11))
                 #                  +ffpc(X3D,xind=tj,npc.max=npc3,splinepars=list(k=11))
                 ,yind=tj)
  respff1pc.fit <- fdata(matrix(respff1pc$fitted.values, ncol = length(tj),
                                byrow = TRUE), argvals = tj)
  ...
  # cv.nonlinear
  resSC.NL1     <- cv.nonlinear(XX, Y1D, t.x.list, tj, s.n.basis = 15, 
                                x.n.basis = 11, t.n.basis = 15,
