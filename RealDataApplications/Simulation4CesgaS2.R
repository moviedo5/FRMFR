...
best.estpred2=function(Y,Ynew,listYhat,listYpred){
nam=names(listYhat)
Mat=matrix(NA,nrow=nrow(Y),ncol=length(listYhat))
Mpred=matrix(NA,nrow=nrow(Ynew),ncol=length(listYpred))
for (i in 1:length(listYhat)){
...
cteeps=function(M,error,rho=0.9){(1-rho)*sum(norm.fdata(M-func.mean(M))^2)/(rho*sum(norm.fdata(error)^2))}
...
dgen=imodelfrfr(imodel=imodel,ivar=ivar,N=N,Ntest=Ntest,nT=nT,nS=nS,rho=rho)
###############
...
YD=dgen$ldatos$Y$data;X1D=dgen$ldatos$X1$data;X2D=dgen$ldatos$X2$data
XX=list(X1=dgen$ldatos$X1$data,X2=dgen$ldatos$X2$data)
XXnew=list(X1=dgen$ldatnew$X1$data,X2=dgen$ldatnew$X2$data)
b.x=list(X1=create.pc.basis(dgen$ldatos$X1,1:nc1),X2=create.pc.basis(dgen$ldatos$X2,1:nc2)) #,X3=create.pc.basis(X3,1:nc3))
b.y=create.pc.basis(dgen$ldatos$Y,1:ncy)

#Estimación de modelos

pmetric=list(df=data.frame(idx=1:nrow(dgen$ldatos$Y)),Y=list(metric=metric.lp,lp=2),X1=list(metric=metric.lp,lp=2),X2=list(metric=metric.lp,lp=2)) #,X3=list(metric=metric.lp,lp=2))
...
ressam=fregre.sam.fr(Y~s(X1,k=kpar)+s(X2,k=kpar),data=dgen$ldatos,basis.y=b.y,basis.x=b.x)
...
respff.NL=pffr(YD~sff(X1D,xind=dgen$ldatos$X1$argvals,splinepars=list(bs="ps",k=c(11,5,5),m=c(2,2,2)))
				 +sff(X2D,xind=dgen$ldatos$X2$argvals,splinepars=list(bs="ps",k=c(11,5,5),m=c(2,2,2))),
				 yind=dgen$ldatos$Y$argvals,bs.yindex=list(bs="ps",k=5,m=c(2,1)))

respffNL.fit=fdata(matrix(respff.NL$fitted.values,ncol=length(dgen$ldatos$Y$argvals),byrow=TRUE),argvals=dgen$ldatos$Y$argvals)
respffpc=pffr(YD~ffpc(X1D,xind=dgen$ldatos$X1$argvals,npc.max=nc1)+ffpc(X2D,xind=dgen$ldatos$X2$argvals,npc.max=nc2),
			yind=dgen$ldatos$Y$argvals)
respffpc.fit=fdata(matrix(respffpc$fitted.values,ncol=length(dgen$ldatos$Y$argvals),byrow=TRUE),argvals=dgen$ldatos$Y$argvals)


prff.NL=fdata(predict(respff.NL,list(X1D=dgen$ldatnew$X1$data,X2D=dgen$ldatnew$X2$data)),argvals=dgen$ldatos$Y$argvals)
prff.pc=fdata(predict(respffpc,list(X1D=dgen$ldatnew$X1$data,X2D=dgen$ldatnew$X2$data)),argvals=dgen$ldatos$Y$argvals)

########## FRegSigCom

t.x.list=list(X1=dgen$ldatos$X1$argvals,X2=dgen$ldatos$X2$argvals)
#----- ---- ---- ----     cv.sigcom
resSC.Lin=cv.sigcom(XX, YD, t.x.list, dgen$ldatos$Y$argvals,s.n.basis=15,t.n.basis=19)
...
#----- ---- ---- ----     cv.nonlinear
resSC.NL=cv.nonlinear(XX, YD, t.x.list, dgen$ldatos$Y$argvals,x.n.basis=11,s.n.basis=15,t.n.basis=19)
